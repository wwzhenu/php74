...
struct _zend_array {
	...
	union {
		struct {
			ZEND_ENDIAN_LOHI_4(
				zend_uchar    flags, // 用各个bit表达HashTable的各种标记
				zend_uchar    _unused,
				zend_uchar    nIteratorsCount,// 迭代器计数 用于foreach
				zend_uchar    _unused2)
		} v;
		uint32_t flags;
	} u;
	uint32_t          nTableMask; // 掩码，一般为-nTableSize
	Bucket           *arData; // 实际存储容器 存储bucket数组
	uint32_t          nNumUsed; // 已使用bucket数量 包括有效bucket和无效bucket 下标从0~nNumUsed-1属于已使用bucket，之后属于未使用bucket
	uint32_t          nNumOfElements;// 有效bucket数量
	uint32_t          nTableSize; // HashTable大小 数组大小 始终是2的n次方，最小是8
	uint32_t          nInternalPointer; // HashTable全局默认游标
	zend_long         nNextFreeElement;// HashTable自然key。默认为0，插入元素，不指定key，会使用此值作为key
